  ...
  describe 'Validation test' do
    context 'If the task Title is an empty string' do
      it 'Validation fails' do
        task = Task.new(title: '', content: 'Create a proposal.', deadline_on: Date.today, priority: :medium, status: :not_started)
    ...
    context 'If the task description is empty' do
      it 'Validation fails' do
        task = Task.new(title: 'Document preparation', content: '', deadline_on: Date.today, priority: :medium, status: :not_started)
        expect(task).not_to be_valid
      end
    end
    ...
    context 'If the task Title and description have values' do
      it 'You can register a task' do
        task = Task.new(title: 'Document preparation', content: 'Create a proposal.', deadline_on: Date.today, priority: :medium, status: :not_started)
